...
function userRegister(req, res, next) {
  ...
  db.none(
    "INSERT INTO users(username,password,firstname,lastname,email,phone)" +
      "VALUES($1,$2,$3,$4,$5,$6)",
    [userName, password, firstName, lastName, emailAddress, phoneNumber]
  )
    .then(function() {
...
function getAllBooks(completion) {
  ...
  db.any(
    "select id,title,author,isbn,availability,imageurl,description from books"
  ).then(function(data) {
...
function addBook(req, res, next) {
  ...
  db.none(
    "INSERT into books(title, author, isbn, availability,imageurl,description,categoryid)" +
      "VALUES($1,$2,$3,$4,$5,$6,$7)",
    [title, author, isbn, availability, imageurl, description, categoryId]
  )
    .then(function() {
...
function getBookById(req, res, next) {
  ...
  db.any("select id,categoryname from categories").then(function(data) {
    //console.log(data)
    data.forEach(function(category) {
      //console.log(book)
      categoriesArray.push(category);
  ...
  db.one(
    "SELECT id,title,author,isbn,availability,imageurl,description from books WHERE id = $1",
    [bookId]
  )
    .then(function(data) {
      //console.log(data)
      //console.log(categoriesArray)
      res.render("admin/updateBooks", {
        bookdata: data,
...
function updateBook(req, res, next) {
  ...
  db.none(
    "UPDATE books SET title=$1, author=$2, isbn=$3, availability=$4,imageurl=$5,description=$6,categoryid=$7 WHERE id=$8",
    [
      req.body.title,
      req.body.author,
      req.body.isbn,
...
function searchBook(req, res, next) {
  ...
  db.any(
    `SELECT title,author,isbn,availability,imageurl,description from books WHERE LOWER(title) LIKE '%${searchText.toLowerCase()}%'`
  )
...
function getAvailableBooks(req, res, next) {
  ...
  db.any(
    "select id,title,author,isbn,availability,imageurl,description from books WHERE availability ='True' "
  )
    .then(function(data) {
      data.forEach(function(book) {
        booksArray.push(book);
...
function checkout(req, res, next) {
  ...
  if (req.session.userId) {
    ...
    db.none("INSERT into userbooks(userid,bookid) VALUES($1,$2)", [
      req.session.userId,
      req.session.currentBookId
    ])
      .then(function() {
        db.none("UPDATE books SET availability ='False' WHERE id =$1", [
          req.session.currentBookId
        ])
          .then(function() {
            route = "/mybooks";
...
function myBooks(req, res, next) {
  ...
  if (req.session.userId) {
    db.any("select bookid from userbooks where userid = $1", [
      req.session.userId
    ])
      .then(function(bookId) {
        ...
        db.any(
          "SELECT books.id,title, author, imageurl,isbn,checkoutdate from books JOIN userbooks on books.id = userbooks.bookid WHERE userbooks.userid = $1",
          [req.session.userId]
        )
          .then(function(books) {
